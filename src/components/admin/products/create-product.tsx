...
import { useCreateProducts, useUpdateProduct } from "../../../hooks/products/use-products";
...
export const ProductForm = memo(({ defaultValues, formTitle, onCancel, isCreateMode, onDelete }: ProductFormProps) => {
  ...
  useEffect(() => {
    if (categories) {
      ...
      setCurrentCategory(
        !category ? null : typeof category === "object" ? category : categories.find(cat => cat.id === category) || null
      );
    }
    ...
    <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-6" id="product-form">
      ...
      <div className="grid grid-cols-1 gap-4">
        ...
        {defaultValues?.images && defaultValues.images.length > 0 && (
          <div className="text-sm text-gray-500">* Ảnh hiện tại sẽ được giữ nguyên nếu bạn không tải lên ảnh mới</div>
        )}
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Input label="Cấp độ bảo vệ (IPxx)" {...register("protectionLevel")} error={errors.protectionLevel?.message} />
      ...
      {defaultValues?.id && (
        <div className="text-sm text-gray-500">
          ...
          {defaultValues.updatedAt && (
            <span className="ml-4">Cập nhật lần cuối: {format(defaultValues.updatedAt, "HH:mm dd/MM/yyyy")}</span>
          )}
        </div>
      )}
      ...
      <div className="flex justify-end items-center gap-4 pt-4 border-t">
        ...
        {!isCreateMode && onDelete && (
          <ConfirmDeleteButton name="Xóa sản phẩm" onConfirm={onDelete} title={`Bạn muốn xóa sản phẩm này không?`} />
        )}
      </div>
    </form>
  );
